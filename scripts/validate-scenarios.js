...
class ScenarioValidator {
  ...
  initializeSchemas() {
    this.metadataSchema = {
      type: 'object',
      required: ['id', 'name', 'version', 'description', 'author', 'gameVersion'],
      properties: {
        id: { type: 'string', pattern: '^[a-z0-9-]+$' },
        name: { type: 'string', minLength: 1 },
        version: { type: 'string', pattern: '^\\d+\\.\\d+\\.\\d+$|^\\d+\\.\\d+$|^v?\\d+\\.\\d+\\.\\d+' },
        description: { type: 'string', minLength: 10 },
        author: { type: 'string', minLength: 1 },
        gameVersion: { type: 'string', pattern: '^\\d+\\.\\d+(\\.\\d+)?$' },
        ...
        requirements: {
          type: 'object',
          properties: {
            minGameVersion: { type: 'string' },
            dlcRequired: { type: 'boolean' }
          }
        },
        compatibility: {
          type: 'object',
  ...
  async validateScenarioFiles(scenarioDir) {
    const files = {
      metadata: { required: true, path: path.join(scenarioDir, 'metadata.json') },
      factions: { required: true, path: path.join(scenarioDir, 'factions.csv') },
      localization: { required: true, path: path.join(scenarioDir, 'Localization.csv') },
      traderConfig: { required: false, path: path.join(scenarioDir, 'TraderNPCConfig.json') },
      prefabs: { required: false, path: path.join(scenarioDir, 'prefabs.json') },
    ...
    for (const [fileKey, fileInfo] of Object.entries(files)) {
      ...
      if (fileInfo.required && !exists) {
        validation.errors.push(`Missing required file: ${path.basename(fileInfo.path)}`);
        validation.isValid = false;
      } else if (!fileInfo.required && !exists) {
        validation.warnings.push(`Optional file not found: ${path.basename(fileInfo.path)}`);
      }
    ...
    if (validation.fileResults.metadata.exists) {
      const metadataResult = await this.validateMetadataFile(files.metadata.path);
      validation.fileResults.metadata = { ...validation.fileResults.metadata, ...metadataResult };
      
      if (!metadataResult.isValid) {
        validation.errors.push(...metadataResult.errors.map(e => `Metadata: ${e}`));
        validation.isValid = false;
      }
    }
    
    if (validation.fileResults.factions.exists) {
      const csvResult = await this.validateCSVStructure(files.factions.path, ['id', 'name']);
      validation.fileResults.factions = { ...validation.fileResults.factions, ...csvResult };
      
      if (!csvResult.isValid) {
        validation.errors.push(...csvResult.errors.map(e => `Factions CSV: ${e}`));
        validation.isValid = false;
      }
    }
    
    if (validation.fileResults.localization.exists) {
      const csvResult = await this.validateCSVStructure(files.localization.path, ['KEY', 'English']);
      validation.fileResults.localization = { ...validation.fileResults.localization, ...csvResult };
      
      if (!csvResult.isValid) {
        validation.errors.push(...csvResult.errors.map(e => `Localization CSV: ${e}`));
        validation.isValid = false;
      }
    }
    ...
    if (validation.fileResults.traderConfig.exists) {
      ...
      validation.fileResults.traderConfig = { ...validation.fileResults.traderConfig, ...jsonResult };
      
      if (!jsonResult.isValid) {
        validation.errors.push(...jsonResult.errors.map(e => `TraderNPCConfig: ${e}`));
        validation.isValid = false;
      }
    }
    ...
    if (validation.fileResults.prefabs.exists) {
      ...
      validation.fileResults.prefabs = { ...validation.fileResults.prefabs, ...jsonResult };
      
      if (!jsonResult.isValid) {
        validation.errors.push(...jsonResult.errors.map(e => `Prefabs: ${e}`));
        validation.isValid = false;
      }
    }
  ...
  printResults() {
    ...
    this.results.summary.forEach(scenario => {
      ...
      console.log(`${status} ${scenario.name} (${scenario.errorCount} errors, ${scenario.warningCount} warnings)`);
      ...
      if (this.verbose) {
        ...
        if (details.warnings.length > 0) {
          details.warnings.forEach(warning => console.log(`   WARNING: ${warning}`));
        }
      }
    });
